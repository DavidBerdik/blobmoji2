...
rm -rf build/* build/.* && time make -j 48
...
# Build the COLRv0 font (slow)

python colrv0_generate_configs.py
git diff colrv0/*.toml

# Compile the fonts
# Should take ~20 minutes
(cd colrv0 && rm -rf build/ && time nanoemoji *.toml)
cp colrv0/build/NotoColorEmoji.ttf fonts/Noto-COLRv0.ttf
cp colrv0/build/NotoColorEmoji-noflags.ttf fonts/Noto-COLRv0-noflags.ttf

# Post-process them
python colrv0_postproc.py
