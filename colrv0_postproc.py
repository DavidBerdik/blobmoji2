"""
Post-nanoemoji processing of the Noto COLRv0 Emoji files.
...
_COLR_FILES = {
    Path("fonts/Noto-COLRv0-noflags.ttf"),
    Path("fonts/Noto-COLRv0.ttf"),
}


def _is_colrv0(font):
    return "COLR" in font and font["COLR"].version == 0
...
def _copy_names(colr_font, cbdt_font):
    ...
    # Amendments
    _set_name(name_table, 10, "Color emoji font using COLRv0.")
...
def _add_vs_cmap(colr_font):
    ...
    add_vs_cmap.modify_font("COLRv0 Emoji", colr_font, "emoji", emoji_variants)
...
def _add_vertical_layout_tables(cbdt_font, colr_font):
    ...
    # to have height=0; but colrv0 font doesn't have that anyway, so I just skip it
...
def _add_fallback_subs_for_unknown_flags(colr_font):
    ...
    assert (
        len(colr_lookups) == 1
    ), f"expected only 1 lookup in COLRv0's GSUB.LookupList, found {len(colr_lookups)}"
    assert (
        colr_lookups[0].LookupType == 4
    ), f"expected Lookup[0] of type 4 in COLRv0, found {colr_lookups[0].LookupType}"
...
def _set_no_font_embedding_restrictions(colr_font):
    ...
    # so the COLRv0 variant must also have no such restrictions.
...
def main(_):
    ...
    for colr_file in _COLR_FILES:
        colr_font = _font(colr_file, _is_colrv0, " must be a COLRv0 font")
        ...
        # Note: COLRv0 does not support gradients or advanced composite modes,
        # so we skip the soft-light effect that is applied to flags in COLRv1
